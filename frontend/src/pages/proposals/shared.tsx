/**
 * Shared UI + formatting primitives for the deal-flow modules
 * (quotes, proposals, insurers, offerings).
 *
 * It lives inside `pages/proposals/` because the proposal is the centre of
 * gravity of this pass and these four page directories are owned by the same
 * module pass — importing `@/pages/proposals/shared` from the sibling modules
 * keeps one copy of the money/deductible/status vocabulary instead of four.
 *
...
export function KeyValue({
  ...
  return (
    <div className={cn("min-w-0", className)}>
      <div className="text-caption uppercase tracking-[0.08em] text-text-muted">{label}</div>
      <div
        className={cn(
...
/** Empty state with an icon chip — used whenever a list comes back with zero rows. */
...
export function ErrorBanner({
  ...
  return (
    <div
      role="alert"
      className={cn(
        "flex items-start gap-2.5 rounded-[12px] border border-[color-mix(in_srgb,var(--red)_35%,transparent)] bg-[color-mix(in_srgb,var(--red)_10%,transparent)] px-3.5 py-2.5",
        className,
      )}
    >
      <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-signal-danger" />
      <p className="text-caption text-text-secondary">{apiError(error, t("state.error"))}</p>
    </div>
  );
}
  ...
  return (
    <DisabledHint hint={reason ?? t("nav.comingSoon")}>
      <Button variant={variant} size={size} disabled className={className}>
        {children}
        <Badge variant="muted" className="ml-1 px-1.5 py-0 text-[9.5px] uppercase">
          {t("nav.soon")}
        </Badge>
      </Button>
    </DisabledHint>
...
export function CopyButton({
  ...
  return (
    <Button variant={variant} size={size} className={className} onClick={() => void copy()}>
      {copied ? <Check className="h-4 w-4 text-lime-deep" /> : <Copy className="h-4 w-4" />}
      {label ?? (copied ? t("shared.copied") : t("shared.copy"))}
    </Button>
  );
}

/** Small mono chip for IDs / RUTs / CMF codes. */
export function MonoChip({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-[7px] border border-line bg-bg-recessed px-1.5 py-0.5 font-mono text-mono-sm text-text-tertiary",
        className,
      )}
    >
      {children}
